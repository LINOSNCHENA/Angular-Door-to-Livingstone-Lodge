...
app.post('/winsertData', function (req, res) {
    ...
    let isactive = req.body.isactive;
    let createdat = new Date().toISOString().substring(0,19).replace('T',' ');
    connection.query("INSERT INTO `workers`( `name`, `department`, `salary`, `isactive`, `createdat`) VALUES('" + name + "','" + department + "','" + salary + "','" + isactive + "','" + createdat + "')", function (err, results) {
        if (err) throw err;
        res.send({ "msg": "Successfully Inserted" });
    });
});
...
app.post('/wupdateData/:id', function (req, res) {
    ...
    let isactive = req.body.isactive;
    let createdat = new Date().toISOString().substring(0,19).replace('T',' ');
    connection.query("UPDATE `workers` SET `name`= '" + name + "', `department` = '" + department + "',`salary` ='" + salary + "',isactive='" + isactive + "',createdat='" + createdat + "'  where `id`= " + id, function (err, results) {
        if (err) throw err;
        res.send({ "msg": "Successfully Updated" });
    });
});
